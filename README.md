...
`metafun` and `metaret` can replace `function` and `return` whenever
the call stack is not needed, which speeds up the code dramatically.

Mutual tail recursion is supported. If you do not
